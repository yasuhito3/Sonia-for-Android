...
一般的なAndroid音楽アプリとSoniaの経路を比較してみてください：
...
音源ファイル（FLAC / WAV / MP3 等）
    ↓
ffmpeg
    ・デコード（圧縮音源 → PCM）
    ・EQ処理（10バンド equalizer）
    ・ゲイン調整
    ↓  OSパイプ（ミドルウェアなし・直結）
mpv
    ・最小限の処理のみ
    ↓
Android AudioFlinger      ← ここだけOSが介在（不可避）
    ↓
...
Soniaは AudioFlinger より上の経路を極限まで短くしました。
特に「リサンプラーを通らない」ことが音質劣化を防ぐ大きな理由です。

---

### Xubuntu24版との比較

Sonia Android版はあくまで「入り口」です。
本格的な高音質再生はXubuntu24版で実現しています：
...
ffmpeg
    ...
    ・アップサンプリング（192kHz / 384kHz）
    ・32bit S32LE 出力
    ・多バンドEQ（ホール残響・ダイナミクス含む）
    ・楽友協会「黄金反射」チューニング
    ↓  OSパイプ（ミドルウェアゼロ）
aplay（ALSA）
    ・hw:X,0 でDACを直接指定
    ・カーネル直結・32bit/192kHz以上
    ↓
サウンドカード / DAC（直接書き込み）
    ↓
スピーカー / ヘッドフォン
...
|------|-----------|-------------|
| ミドルウェア | AudioFlingerのみ | **完全にゼロ** |
| ビット深度 | 端末依存 | **32bit S32LE** |
| サンプルレート | 端末依存 | **192kHz / 384kHz** |
| DAC指定 | 不可 | **hw:X,0 直接指定** |
| EQバンド数 | 10バンド | **多バンド＋ホール残響** |
