...
function onVol(v) {{
  ...
  _volTimer = setTimeout(() => {{ sendVol(v); }}, 300);
}}
...
const _volEl = document.getElementById('vol-sl');
_volEl.addEventListener('touchend',  () => {{ clearTimeout(_volTimer); sendVol(_volEl.value); }}, {{passive:true}});
_volEl.addEventListener('pointerup', () => {{ clearTimeout(_volTimer); sendVol(_volEl.value); }});
